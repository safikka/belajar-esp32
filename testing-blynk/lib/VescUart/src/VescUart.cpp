...
void VescUart::gateOpenFull(){
	int32_t index = 0;
	uint8_t payload[5];

	payload[index++] = COMM_GATE_OPEN_FULL;

	packSendPayload(payload, 5);
}

void VescUart::gateOpenHalf(){
	int32_t index = 0;
	uint8_t payload[5];

	payload[index++] = COMM_GATE_OPEN_HALF;

	packSendPayload(payload, 5);
}

void VescUart::gateOpenPeople(){
	int32_t index = 0;
	uint8_t payload[5];

	payload[index++] = COMM_GATE_OPEN_PEOPLE;

	packSendPayload(payload, 5);
}

void VescUart::gateClose(){
	int32_t index = 0;
	uint8_t payload[5];

	payload[index++] = COMM_GATE_CLOSE;
